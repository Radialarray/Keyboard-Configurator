...
fn test_layer_refs_simple_mo() {
    ...
    assert!(stdout.contains("Layer 1: Function"), "Should show layer 1");
    assert!(
        stdout.contains("Inbound References:"),
...
fn test_layer_refs_json_output() {
    ...
    for layer in layers {
        ...
        assert!(
            layer["inbound_refs"].is_array(),
            "inbound_refs should be array"
        );
    ...
    let ref0 = &refs[0];
    assert!(
        ref0["from_layer"].is_number(),
        "from_layer should be number"
    );
...
fn test_layer_refs_transparency_conflict() {
    ...
    assert!(
        stdout.contains("KC_A"),
        "Should mention conflicting keycode"
    );
}
...
fn test_layer_refs_all_types() {
    ...
    assert!(
        stdout.contains("Toggle") || stdout.contains("TG"),
        "Should show Toggle"
    );
    assert!(
    ...
    assert!(
        stdout.contains("Switch") || stdout.contains("TO"),
        "Should show Switch"
    );
    assert!(
