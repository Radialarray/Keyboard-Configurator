...
impl<'a> FirmwareGenerator<'a> {
    ...
    fn generate_layer_keys_by_led(&self, layer: &crate::models::layer::Layer) -> Result<Vec<String>> {
    ...
    fn generate_vial_json(&self) -> Result<String> {
        ...
        let json_str = serde_json::to_string_pretty(&vial_config)
            .context("Failed to serialize vial.json")?;
    ...
    fn write_keymap_c(&self, content: &str) -> Result<String> {
        ...
        fs::rename(&temp_path, &keymap_path)
            .with_context(|| format!("Failed to rename {} to {}", temp_path.display(), keymap_path.display()))?;
    ...
    fn write_vial_json(&self, content: &str) -> Result<String> {
        ...
        fs::rename(&temp_path, &vial_path)
            .with_context(|| format!("Failed to rename {} to {}", temp_path.display(), vial_path.display()))?;
