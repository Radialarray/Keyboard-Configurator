...
fn test_tap_dance_empty_vec_serde_behavior() {
    ...
    assert!(
        !yaml.contains("tap_dances"),
        "Empty tap_dances should not appear in YAML"
    );
}
...
fn test_td_inline_wizard_three_way_flow() {
    ...
    assert!(final_keycode.is_some());
    assert_eq!(
        final_keycode.unwrap(),
        "TD(shift_caps, KC_LSFT, KC_CAPS, KC_RSFT)"
    );
}
...
fn test_pending_keycode_build_formats_correctly() {
    ...
    assert_eq!(
        pending.build_keycode(),
        Some("TD(test, KC_A, KC_B)".to_string())
    );
    ...
    assert_eq!(
        pending.build_keycode(),
        Some("LT(@layer1, KC_SPC)".to_string())
    );
    ...
    assert_eq!(
        pending.build_keycode(),
        Some("MT(MOD_LSFT, KC_ENT)".to_string())
    );
}
