...
impl<'a> FirmwareGenerator<'a> {
    ...
    fn generate_keymap_c(&self) -> Result<String> {
        ...
        // Add RGB matrix indicators for layout-driven colors (when RGB_MATRIX_ENABLE is defined)
        code.push('\n');
        code.push_str(&self.generate_rgb_matrix_indicators_user()?);
    ...
    /// Generates per-layer RGB colors ordered by LED index.
    ///
    /// Uses the same visual -> matrix -> LED mapping as keycodes and
    /// applies Layout::resolve_key_color to each key.
    ...
    /// Generates RGB matrix indicator function and color tables.
    ///
    /// This emits C code guarded by #ifdef RGB_MATRIX_ENABLE that sets
    /// per-LED colors based on the active layer.
    fn generate_rgb_matrix_indicators_user(&self) -> Result<String> {
        // If there is no RGB matrix geometry, skip emitting anything.
        ...
        code.push_str("// Layout-driven RGB colors generated by keyboard_tui\n");

        let led_count = self.mapping.key_count();

        // Emit color table: one row per layer, one entry per LED.
        code.push_str("const rgb_t PROGMEM layer_colors[][RGB_MATRIX_LED_COUNT] = {\n");
        ...
        for (layer_idx, _layer) in self.layout.layers.iter().enumerate() {
            ...
            for (idx, color) in colors.iter().enumerate() {
                code.push_str(&format!("{{{}, {}, {}}}", color.r, color.g, color.b));
                if idx < led_count - 1 {
                    code.push_str(", ");
                }

                if (idx + 1) % 6 == 0 && idx < led_count - 1 {
                    code.push_str("\n        ");
                }
            }
        ...
        // Emit indicators function that applies the colors for the
        // highest active layer. We intentionally keep logic simple
        // and let users customize further if desired.
        ...
        code.push_str(
            "        rgb_matrix_set_color(i, layer_colors[layer][i].r, layer_colors[layer][i].g, layer_colors[layer][i].b);\n",
        );
        code.push_str("    }\n\n");
        code.push_str("    return false;\n");
        code.push_str("}\n\n");
...
mod tests {
    ...
    fn create_color_test_setup() -> (Layout, KeyboardGeometry, VisualLayoutMapping, Config) {
        let mut layout = Layout::new("Color Test").unwrap();

        // Layer 0: default white, with an override and a category color
        let mut layer0 = Layer::new(0, "Base", RgbColor::new(255, 255, 255)).unwrap();
        let key0 = KeyDefinition::new(Position::new(0, 0), "KC_A")
            .with_color(RgbColor::new(255, 0, 0));
        let key1 = KeyDefinition::new(Position::new(0, 1), "KC_B");
        layer0.add_key(key0.clone());
        layer0.add_key(key1.clone());

        // Layer 1: different default color
        let mut layer1 = Layer::new(1, "Fn", RgbColor::new(0, 0, 255)).unwrap();
        let key0_l1 = KeyDefinition::new(Position::new(0, 0), "KC_A");
        let key1_l1 = KeyDefinition::new(Position::new(0, 1), "KC_B");
        layer1.add_key(key0_l1.clone());
        layer1.add_key(key1_l1.clone());
    ...
    fn test_generate_layer_colors_by_led() {
        let (layout, geometry, mapping, config) = create_color_test_setup();
        let generator = FirmwareGenerator::new(&layout, &geometry, &mapping, &config);

        // Layer 0 colors
        let colors_l0 = generator.generate_layer_colors_by_led(0).unwrap();
        assert_eq!(colors_l0.len(), 2);
        assert_eq!(colors_l0[0], RgbColor::new(255, 0, 0));
        assert_eq!(colors_l0[1], RgbColor::new(255, 255, 255));

        // Layer 1 colors
        let colors_l1 = generator.generate_layer_colors_by_led(1).unwrap();
        assert_eq!(colors_l1.len(), 2);
        assert_eq!(colors_l1[0], RgbColor::new(0, 0, 255));
        assert_eq!(colors_l1[1], RgbColor::new(0, 0, 255));
    }

    #[test]
    fn test_generate_rgb_matrix_indicators_user_included_when_rgb_present() {
        let (layout, geometry, mapping, config) = create_color_test_setup();
        let generator = FirmwareGenerator::new(&layout, &geometry, &mapping, &config);

        let rgb_code = generator.generate_rgb_matrix_indicators_user().unwrap();

        assert!(rgb_code.contains("#ifdef RGB_MATRIX_ENABLE"));
        assert!(rgb_code.contains("const rgb_t PROGMEM layer_colors"));
        assert!(rgb_code.contains("bool rgb_matrix_indicators_user(void)"));
    }

    #[test]
    fn test_generate_rgb_matrix_indicators_user_empty_without_rgb() {
        let (layout, _geometry, _mapping, mut config) = create_color_test_setup();

        // Geometry with no keys -> no RGB matrix
        let geometry = KeyboardGeometry::new("test", "LAYOUT", 0, 0);
        let mapping = VisualLayoutMapping::build(&geometry);

        let generator = FirmwareGenerator::new(&layout, &geometry, &mapping, &config);

        let rgb_code = generator.generate_rgb_matrix_indicators_user().unwrap();
        assert!(rgb_code.is_empty());
    }
}
