...
use keyboard_tui::models::{
    Category, KeyDefinition, KeyboardGeometry, Layer, Layout, LayoutMetadata, Position, RgbColor,
    VisualLayoutMapping, KeyGeometry,
};
...
fn create_test_config(temp_dir: &TempDir) -> Config {
    ...
    let keyboards_dir = qmk_path.join("keyboards").join("test_kb").join("keymaps").join("test_keymap");
...
fn test_validation_invalid_keycode() {
    ...
    // Assert
    assert!(!report.is_valid(), "Layout with invalid keycode should fail validation");
...
fn test_validation_missing_position() {
    ...
    // Assert
    assert!(!report.is_valid(), "Layout with missing position should fail validation");
}
...
fn test_generation_creates_files() {
    ...
    assert!(PathBuf::from(&keymap_path).exists(), "keymap.c should be created");
    assert!(PathBuf::from(&vial_path).exists(), "vial.json should be created");
}
...
fn test_generation_keymap_c_structure() {
    ...
    let content = fs::read_to_string(&keymap_path)
        .expect("Should be able to read keymap.c");

    // Assert - Check for expected C code structure
    assert!(content.contains("// Generated by keyboard_tui"), "Should have generator comment");
    assert!(content.contains("const uint16_t PROGMEM keymaps"), "Should have PROGMEM keymaps array");
    assert!(content.contains("LAYOUT_test"), "Should use correct layout macro");
    assert!(content.contains("KC_0"), "Should contain keycodes from layer 0");
    assert!(content.contains("KC_TRNS"), "Should contain transparent keycodes from layer 1");
    
    // Check for layer comments
    assert!(content.contains("// Layer 0: Base"), "Should have layer 0 comment");
    assert!(content.contains("// Layer 1: Function"), "Should have layer 1 comment");
}
...
fn test_generation_led_ordering() {
    ...
    let layer0_start = content.find("// Layer 0: Base")
        .expect("Should find Layer 0 comment");
    let layer0_layout = &content[layer0_start..];
    let layout_start = layer0_layout.find("LAYOUT_test(")
...
fn test_generation_atomic_write() {
    ...
    assert!(result2.is_ok(), "Second generation should succeed (overwrite)");
    
    // Assert - Files should exist and be readable
    let (keymap_path, vial_path) = result2.unwrap();
    let keymap_content = fs::read_to_string(&keymap_path)
        .expect("Should read keymap.c after overwrite");
    let vial_content = fs::read_to_string(&vial_path)
        .expect("Should read vial.json after overwrite");
...
fn test_full_pipeline_validation_to_generation() {
    ...
    assert!(report.is_valid(), "Layout should be valid before generation");
    ...
    // Assert
    assert!(result.is_ok(), "Generation should succeed after validation: {:?}", result.err());
    
    let (keymap_path, vial_path) = result.unwrap();
    assert!(PathBuf::from(&keymap_path).exists(), "keymap.c should exist");
    ...
    assert!(keymap_content.len() > 100, "keymap.c should have substantial content");
    assert!(vial_content.len() > 50, "vial.json should have substantial content");
    
    // Verify vial.json is valid JSON
    let _: serde_json::Value = serde_json::from_str(&vial_content)
        .expect("vial.json should be valid JSON");
}
